...
void recorrerAutos(struct nodoAutos **autos, int precioNuevo, char modelo[100]){
    if(autos!=NULL){
        recorrerAutos(&((*autos)->sig), precioNuevo, modelo);
...
void recorrerMarcas(struct Marca **marcas, int precioNuevo, char modelo[100])
{
    if(marcas!=NULL){
        recorrerMarcas(&((*marcas)->sig), precioNuevo, modelo);
        recorrerAutos(&((*marcas)->autos), precioNuevo, modelo);
    }
}
void recorrerSucursales(struct plantasDistribucion **sucursales, int nuevoPrecio, char modelo[100])
{
    if(sucursales!=NULL){
        recorrerSucursales(&((*sucursales)->izq), nuevoPrecio, modelo);
        recorrerSucursales(&((*sucursales)->der), nuevoPrecio, modelo);
        recorrerMarcas(&((*sucursales)->datosArbol->autos),nuevoPrecio,modelo);
    }
}
void recorrerComunas(struct listaComunas **comunas, int nuevoPrecio, char modelo[100])
{
    if(comunas!=NULL){
        recorrerComunas(&((*comunas)->sig), nuevoPrecio, modelo);
        recorrerSucursales(&((*comunas)->datosComuna->Sucursales), nuevoPrecio, modelo);
    }
}
void recorrerRegiones(struct nodoRegiones **regiones, int nuevoPrecio, char modelo[100])
{
    if(regiones!=NULL){
        recorrerRegiones(&((*regiones)->sig), nuevoPrecio, modelo);
        recorrerComunas(&((*regiones)->comunas), nuevoPrecio, modelo);
    }
}
void cambiarValorAutoSucursales(struct DercoDistribuidora **derco, int nuevoPrecio, char modelo[100])
{
    if((*derco)->regiones!=NULL){
        ...
        recorrerRegiones(&(recRegiones), nuevoPrecio, modelo);
    }
}
void cambiarValorAuto(int nuevoPrecio, char modelo[100],struct DercoDistribuidora **derco){
    if((*derco)->stockGeneral->autosEnStock->autos!=NULL){
        ...
        recorrerMarcas(&recMarcas, nuevoPrecio, modelo);
...
void recNuevoValorPlantas(struct plantasDistribucion *plantas, int nuevoValor, int idPlanta, int *esta)
{
    if(plantas!=NULL) {
        recNuevoValorPlantas(plantas->izq, nuevoValor, idPlanta, esta);
        recNuevoValorPlantas(plantas->der, nuevoValor, idPlanta, esta);
...
void cambiarVentasDePlanta(struct DercoDistribuidora **derco, int nuevoValor, int idPlanta)
{
    ...
    while(recRegiones!=NULL){
        ...
        while(recComunas!=NULL){
            recNuevoValorPlantas(recComunas->datosComuna->Sucursales, nuevoValor, idPlanta, &esta);
            recComunas=recComunas->sig;
        }
        recRegiones=recRegiones->sig;
    }
...
int ingresarVehiculoAbb(struct plantasDistribucion **headAbb, int idBuscado){
...
void menuCambiar(struct DercoDistribuidora *derco){
    ...
    switch(opcion){
        case 1:
            ...
            int nuevoValor;
            scanf("%d",&nuevoValor);
            cambiarValorAuto(nuevoValor,buffer,&derco);///OK
            break;
        case 2:
